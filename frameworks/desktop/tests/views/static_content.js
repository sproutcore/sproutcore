...
test("Frame Recalculation", function() {
  ...
  var view = SC.StaticContentView.create({
   ...
   pane.adjust('width', 499);
   equals(callCount, baseCount+1, 'frame should recompute after parent resizes');
   ...
   equals(callCount, baseCount+2, 'frame should recompute after content changes');
   SC.RunLoop.begin().end();
   ok(view.get('layer').textContent.indexOf(content) > -1, 'view should rerender when content changes');
   view.contentLayoutDidChange();
   ok(callCount > baseCount+2, 'frame should recompute after calling contentLayoutDidChange()');
