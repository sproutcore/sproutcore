...
SC.SplitView = SC.View.extend({
  ...
  */
  ...
  layoutDirectionDidChange: function () {
    ...
    // We have to notify each child that its layout will have changed.
    // TODO: You'd think this wouldn't be necessary.
    childViews.invoke('splitChildLayoutDidChange');
    this.scheduleTiling();
  ...
  */
  ...
  createChildViews: function () {
    ...
    //@if(debug)
    // Provide some migration support (debug mode only).
    var childViews = this.get('childViews');
    if (this.get('topLeftView') && !childViews.contains('topLeftView') ||
      this.get('bottomRightView') && !childViews.contains('bottomRightView')) {
      SC.error("Developer Error: SC.SplitView has been updated to accept multiple children and no longer comes with topLeftView and bottomRightView by default.  You must specify the childViews that you wish to use, such as 'childViews: ['leftPanel', 'centerPanel', 'rightPanel']'.");
    }
    //@endif
    if (this._scsv_settingUpChildViews) return;
    this._scsv_settingUpChildViews = YES;
