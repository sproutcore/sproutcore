...
module('SC.Buildable', {
  setup: function() {
    parent = SC.View.create(SC.BuildChildren, {
      childDidBuildIn: function() {
        this.called_childDidBuildIn = YES;
      },
      childDidBuildOut: function() {
        this.called_childDidBuildOut = YES;
      }
    });
...
test("Calling buildInChild adds child and builds it in.", function(){
  ...
  ok(!p.called_childDidBuildIn, "child has not finished building in, according to parent.");
  ...
  ok(p.called_childDidBuildIn, "child has finished building in, according to parent.");
});
...
test("Calling buildOutChild builds out the child, and only removes it when done.", function() {
  ...
  ok(!p.called_childDidBuildOut, "child has not finished building out, according to parent.");
  equals(v.get("parentView"), p, "Parent view should still be the former parent");
  
  // finish build out
  v.didFinishBuildOut();
  
  ok(p.called_childDidBuildOut, "child has finished building out, according to parent.");
