...
test("Adding a task should not cause it to run.",function(){
  var task = SC.Task.create({ startTask: function(){ this.ran = YES; } });
  taskQueue.push(task);
...
test("Adding a task and calling startTask() should cause the task to be run.",function(){
  var task = SC.Task.create({ startTask: function(){ this.ran = YES; } });
  taskQueue.push(task);
  taskQueue.run();
...
test("Adding multiple tasks and calling startTask should run the tasks in order.",function(){
  var ri = 0;
  var task = SC.Task.extend({ startTask: function(){ this.ran = ri++; } }), t1, t2, t3, t4;
