  ...
  */
  ...
  fetch: function(store, query) {
    ...
    if (query.get('location') !== SC.Query.LOCAL) {
      throw new Error(SC.$error('SC.Fixture data source can only fetch local queries'));
    }

    if (!query.get('recordType') && !query.get('recordTypes')) {
      throw new Error(SC.$error('SC.Fixture data source can only fetch queries with one or more record types'));
    }
