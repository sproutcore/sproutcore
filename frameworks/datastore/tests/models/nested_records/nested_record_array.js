...
var initModels = function() {
  ...
  NestedRecord.ParentRecordTest = SC.Record.extend({
    ...
    name : SC.Record.attr(String),
    elements : SC.Record.toMany('SC.Record', {
      nested : true
    }),
...
test("Basic Read", function() {
  ...
  // Check to see if the attributes of a Child Record match the reference of the
  // parent
...
test("Basic Array Functionality: pushObject w/ ChildRecord", function() {
  ...
  elements = testParent.get('elements');
  // PushObject Tests
  cr = store.createRecord(NestedRecord.ChildRecordTest1, {
    type : 'ChildRecordTest1',
    name : 'Testikles',
    value : 'God Of Fertility'
  });
