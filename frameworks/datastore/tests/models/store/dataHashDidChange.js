...
test("called with existing storeKey", function() {
  ...
  ok(!store.chainedChanges || !store.chainedChanges.contains(storeKey), 'precond -chainedChanges should not include storeKey');
  ...
  ok(store.chainedChanges.contains(storeKey), 'chainedChanges should include storeKey');
