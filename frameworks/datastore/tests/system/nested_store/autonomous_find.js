  ...
  setup: function() {
    ...
    MyApp.DataSource = SC.DataSource.create({
      ...
      fetchEquals: function(store, query, count, desc) {
        ...
        // Since this is a nested store the query will use the parent store
        equals(MyApp.store, store, desc + ': should get parent store');
        equals(MyApp.parent, this.store, desc + ': should get nested store');
    ...
    MyApp.parent = SC.Store.create().from(MyApp.DataSource);
    MyApp.store = MyApp.parent.chainAutonomousStore();
    ...
    // load some data
    MyApp.DataSource.storeKeys = MyApp.parent.loadRecords(MyApp.Foo, records);
    SC.RunLoop.end();

    SC.RunLoop.begin();
    // for sanity check, load two record types
    MyApp.parent.loadRecords(MyApp.Bar, records);
    SC.RunLoop.end();
