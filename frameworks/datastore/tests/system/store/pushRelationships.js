...
/**
 * pushRetreive: Dirty State Tests
 */

/**
 * pushRetrieve should push in datahashes in ANY state
 */
test("Propagate relationship in DIRTY states does propagate [one(master) to one(*)]", function () {
  MyApp.TestRecord = SC.Record.extend({
    aValue: SC.Record.attr(String),
    hasAValue: false,

    _aValueDidChange: function() {
      this.set('hasAValue',!!this.get('aValue'));
    }.observes('aValue')
  });

  SC.RunLoop.begin();
  MyApp.store.loadRecords(MyApp.TestRecord, [
    { guid: 'r1' }
  ]);
  SC.RunLoop.end();

  var r1 = MyApp.store.find(MyApp.TestRecord, 'r1');

  ['CLEAN',
   'DIRTY',
   ['EMPTY', 'READY_CLEAN'], // special
   ['ERROR', 'READY_CLEAN'], // special
   'READY',
   'READY_CLEAN',
   'READY_DIRTY',
   'READY_NEW',
   'DESTROYED',
   ['DESTROYED_CLEAN', 'READY_CLEAN'], // special
   'DESTROYED_DIRTY',
   'BUSY',
   'BUSY_LOADING',
   'BUSY_CREATING',
   ...
   'BUSY_REFRESH_CLEAN',
   'BUSY_REFRESH_DIRTY',
   'BUSY_DESTROYING'].forEach(function(statusName) {

     var expectedStatusName = statusName;
     if(SC.typeOf(statusName) == SC.T_ARRAY) {
       expectedStatusName = statusName[1];
       statusName = statusName[0];
     }

     equals(r1.get('status'), SC.Record.READY_CLEAN,
            'precond (%@) - r1 should be in READY_CLEAN'.fmt(statusName));
     ok(SC.none(r1.get('aValue')),
        'precond (%@) - r1 should have NOT have aValue'.fmt(statusName));
     ok(r1.get('hasAValue') === false,
        'precond observer (%@) - r1 should have hasAValue as false'.fmt(statusName));

     SC.RunLoop.begin();
     MyApp.store.writeStatus(r1.get('storeKey'),SC.Record[statusName]);
     MyApp.store.pushRetrieve(MyApp.TestRecord, 'r1', { guid: 'r1', aValue: 'foo' });
     SC.RunLoop.end();

     equals(r1.get('status'), SC.Record[expectedStatusName],
            'precond (%@) - r1 should be in %@'.fmt(statusName,expectedStatusName));
     equals(r1.get('aValue'), 'foo',
            'test value (%@) - r1 should have aValue foo'.fmt(statusName));
     ok(r1.get('hasAValue'),
        'test observer (%@) - r1 should have hasAValue true'.fmt(statusName));

     SC.RunLoop.begin();
     MyApp.store.writeStatus(r1.get('storeKey'),SC.Record.READY_CLEAN);
     MyApp.store.pushRetrieve(MyApp.TestRecord, 'r1', { guid: 'r1' });
     SC.RunLoop.end();
  });
});
