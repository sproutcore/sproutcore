...
test("recordDidChange", function() {
  ...
  try{
    ...
    equals(SC.Record.BUSY_ERROR, error1.message, "the status shouldn't have changed.");
  }
  
  try{
    store.recordDidChange(undefined, undefined, storeKey2);
  }catch(error2){
    equals(SC.Record.NOT_FOUND_ERROR, error2.message, "the status shouldn't have changed.");
  }
