  ...
  VERTICAL_STACK: {
    ...
    adjustChildViews: function (view) {
      var childViews = view.get('childViews'),
        ...
        fitLastRow = options.fitLastRow || false,
      ...
      console.log("%@ - adjustChildViews".fmt(view));
      for (i = 0, len = childViews.get('length'); i < len; i++) {
        ...
        if (SC.none(layout.height) && (i < len - 1 || !fitLastRow)) {
      ...
      if (!fitLastRow && view.getPath('layout.height') !== position) {
        view.adjust('height', position + paddingAfter);
      }
    },

    beginObservingChildViews: function (view) {
      ...
      for (i = 0, len = childViews.get('length'); i < len; i++) {
        ...
        if (!childView.get('useAbsoluteLayout') && !childView.get('useStaticLayout')) {
          // childView.addObserver('layout', view, view.layoutDidChange);
          childView.addObserver('marginTop', view, view.layoutDidChange);
          childView.addObserver('marginBottom', view, view.layoutDidChange);
          childView.addObserver('isVisible', view, view.layoutDidChange);
        }
      }
    }
