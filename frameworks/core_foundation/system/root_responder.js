...
SC.RootResponder = SC.Object.extend(
      ...
      SC.run(function () {
        ...
        if (!SC.TABBING_ONLY_INSIDE_DOCUMENT && !SC.browser.isIE8OrLower) {
          ...
          if (keyPane) {
            ...
            if (nextValidKeyView) {
              keyPane.makeFirstResponder(nextValidKeyView);
            }
          }
        }
  ...
  */
  setup: function () {
    ...
    if (SC.browser.isIE8OrLower) {
      this.listenFor(['focusin', 'focusout'], document);
    } else {
  ...
  */
  keydown: function (evt) {
    ...
    if (SC.browser.isMozilla && (evt.which === 8)) {
      return true;
    }
    ...
    if (this._isFunctionOrNonPrintableKey(evt)) {
      ...
      if (keyCode >= 37 && keyCode <= 40 && SC.browser.isMozilla) {
        return YES;
      }
