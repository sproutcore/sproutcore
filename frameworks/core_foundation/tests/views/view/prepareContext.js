...
test("adds aria-disabled attribute if view isEnabled = NO", function() {
  ...
  view.set('isEnabled', NO);
  view.prepareContext(context, YES);
  ok(context.attrs()['aria-disabled'], 'should have the aria-disabled attribute');

  view.set('isEnabled', YES);
  view.prepareContext(context, YES);
  ok(context.attrs()['aria-disabled'] === undefined, 'should NOT have the aria-disabled attribute');
