...
function transitionFor(view) {
  return styleFor(view)['transition'];
}
...
test("should handle transform attributes", function () {
  ...
  setTimeout(function () {
    equals(transitionFor(view), 'transform' + ' 1s ease 0s', 'add transition');
    equals(styleFor(view)['transform'], 'rotateX(45deg)', 'has both transforms');
...
test("should handle conflicting transform animations", function () {
  ...
  setTimeout(function () {
    ...
    equals(transitionFor(view), 'transform' + ' 1s ease 0s', 'use duration of first');
    equals(styleFor(view)['transform'], 'rotateX(45deg) scale(2)');
    equals(45, view.get('layout').rotateX, 'rotateX is 45deg');
...
test("handles acceleration when appropriate", function () {
  ...
  setTimeout(function () {
    equals(transitionFor(view), 'transform' + ' 1s ease 0s', 'transition is on transform');
...
test("combines accelerated layer animation with compatible transform animations", function () {
  ...
  setTimeout(function () {
    var transform = styleFor(view)['transform'];
...
test("should not use accelerated layer if other transforms are being animated at different speeds", function () {
  ...
  setTimeout(function () {
    ...
    equals(style['transform'], 'rotateX(45deg)', 'transform should only have rotateX');
...
test("Test that cancelAnimation() removes the animation style and fires the callback with isCancelled set.", function () {
  ...
  setTimeout(function () {
    SC.run(function () {
      ...
      equals(transitionFor(view), 'transform' + ' 0.5s ease 0s, width 0.5s ease 0s', 'Tests the CSS transition property');
...
test("Test that cancelAnimation(SC.LayoutState.CURRENT) removes the animation style, stops at the current position and fires the callback with isCancelled set.", function () {
  ...
  setTimeout(function () {
    SC.run(function () {
      ...
      equals(transitionFor(view), 'transform' + ' 1s ease 0s, width 1s ease 0s', 'Tests the CSS transition property');
