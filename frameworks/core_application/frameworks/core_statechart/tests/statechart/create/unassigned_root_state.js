...
var obj3, rootSubstate3, stateE, rootSubstateExample;
var obj4;
var owner;
...
module("SC.Statechart: Create Statechart with Unassigned Root State Tests", {
  setup: function() {
    ...
    obj1 = SC.Object.extend(SC.StatechartManager, {
      ...
      stateA: SC.State.design({
        foo: function() {
          this.gotoSubstate('stateB');
        }
      }),

      stateB: SC.State.design({
        bar: function() {
          this.gotoSubstate('stateA');
        }
      })
    ...
    obj2 = SC.Object.extend(SC.StatechartManager, {
      ...
      stateC: SC.State.design({
        foo: function() {
          this.gotoSubstate('stateD');
        }
      }),

      stateD: SC.State.design({
        bar: function() {
          this.gotoSubstate('stateC');
        }
      })
    ...
    rootSubstateExample = SC.State.extend({ test: 'foo' });

    obj3 = SC.Object.extend(SC.StatechartManager, {
      owner: owner,
      initialState: 'stateE',
      rootSubstateExample: rootSubstateExample,
      stateE: SC.State.design()
    });

    obj3 = obj3.create();
    rootSubstate3 = obj3.get('rootSubstate');
  ...
  teardown: function() {
    obj1 = rootSubstate1 = stateA = stateB = null;
    obj2 = rootSubstate2 = stateC = stateD = null;
    obj3 = rootSubstate3 = stateE = rootSubstateExample = null;
    obj4 = null;
  }
});
...
test("check obj1 statechart", function() {
  ...
  ok(SC.kindOf(rootSubstate1, SC.State), "root state should be kind of SC.State");
  ok(!rootSubstate1.get('substateAreConcurrent'), "root state's substates should not be concurrent");

  equals(obj1.get('initialState'), stateA, "obj's initialState should be state A");
  equals(rootSubstate1.get('initialSubstate'), stateA, "root state's initialState should be state A");
  equals(stateA, rootSubstate1.getSubstate('stateA'), "obj.stateA and rootSubstate.stateA should be equal");
  equals(stateB, rootSubstate1.getSubstate('stateB'), "obj.stateB and rootSubstate.stateB should be equal");

  equals(rootSubstate1.get('owner'), obj1, "root state's owner should be obj");
...
test("check obj2 statechart", function() {
  ...
  ok(SC.kindOf(rootSubstate2, SC.State), "root state should be kind of SC.State");
  ok(rootSubstate2.get('substatesAreConcurrent'), "root state's substates should be concurrent");

  equals(obj2.get('initialState'), null, "obj's initialState should be null");
  equals(rootSubstate2.get('initialSubstate'), null, "root state's initialState should be null");
  equals(stateC, rootSubstate2.getSubstate('stateC'), "obj.stateC and rootSubstate.stateC should be equal");
  equals(stateD, rootSubstate2.getSubstate('stateD'), "obj.stateD and rootSubstate.stateD should be equal");

  equals(rootSubstate2.get('owner'), obj2, "root state's owner should be obj");
...
test("check obj3 statechart", function() {
  ...
  ok(SC.kindOf(rootSubstate3, rootSubstateExample), "root state should be kind of rootSubstateExample");
  ok(!rootSubstate3.get('substatesAreConcurrent'), "root state's substates should be concurrent");

  equals(rootSubstate3.get('owner'), owner, "root state's owner should be owner");
  ...
  equals(rootSubstate3.get('initialSubstate'), stateE, "root state's initialState should be stateE");
  equals(stateE, rootSubstate3.getSubstate('stateE'), "obj.stateE and rootSubstate.stateE should be equal");
...
test("check obj4 statechart", function() {
  ...
  equals(obj4.get('rootSubstate'), null, "obj's root state should be null");

  obj4.initStatechart();

  ok(obj4.get('statechartIsInitialized'), "obj should be an initialized statechart");
  ok(obj4.get('rootSubstate'), "obj's root state should not be null");
  equals(obj4.get('rootSubstate').getSubstate('stateF'), obj4.getState('stateF'), "obj.stateF should be equal to rootSubstate.stateF");
});
