...
var obj1, rootSubstate1, stateA, stateB, stateX, stateY, stateZ;
var obj2, rootSubstate2, stateC, stateD;
var obj3, rootSubstate3, stateE, stateF;
var owner, owner2;
...
module("SC.Statechart: Change Statechart Owner Property Tests", {
  setup: function() {
    ...
    obj3 = TestObject.extend({
      ...
      stateE: TestState.design({
        foo: function() {
          this.gotoSubstate('stateF');
        }
      }),

      stateF: TestState.design({
        bar: function() {
          this.gotoSubstate('stateE');
        }
      })
  ...
  teardown: function() {
    obj1 = rootSubstate1 = stateA = stateB = stateX = stateY = stateZ = null;
    obj2 = rootSubstate2 = stateC = stateD = null;
    obj3 = rootSubstate3 = stateE = stateF = null;
    owner = owner2 = null;
...
test("check obj1 -- basic owner get and set", function() {
  ...
  equals(rootSubstate1.get('owner'), owner, "root state's owner should be owner");
...
test("check obj2 -- statechartOwnerKey", function() {
  equals(rootSubstate2.get('owner'), owner, "root state's owner should be owner");
...
test("check obj3 -- basic owner get and set", function() {
  ...
  equals(rootSubstate3.get('owner'), obj3, "root state's owner should be obj");
  ...
  equals(rootSubstate3.get('owner'), owner2, "root state's owner2 should be owner2");
  ...
  equals(rootSubstate3.get('owner'), owner2);

  obj3.destroy();

  ok(!obj3.hasObserverFor('fooOwner'));
  equals(rootSubstate3.get('owner'), null);
});
