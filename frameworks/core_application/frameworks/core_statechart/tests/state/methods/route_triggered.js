    ...
    sc = SC.Statechart.create({
      ...
      foo: SC.State.design({
        ...
        createAppSubstateRouteHandlerContext: function(attr) {
          this.info.createAppSubstateRouteHandlerContext = {
            attr: attr
          };
          return sc_super();
        },
...
test("invoke routeTriggered where delegate does allow state to handle route", function() {
  ...
  info = foo.info.createAppSubstateRouteHandlerContext;

  ok(info, "state.createAppSubstateRouteHandlerContext should have been invoked");
  ok(info.attr, "state.createAppSubstateRouteHandlerContext should be provided attr param");
  ...
  ok(SC.kindOf(context, SC.AppSubstateRouteHandlerContext), "state.handleTriggeredRoute should be provided a state route handler context object");
  ...
  ok(SC.kindOf(context, SC.AppSubstateRouteHandlerContext), "state.statechartShouldStateHandleTriggeredRoute should be provided a state route handler context object");
  equals(context.get('state'), foo, "context.state should be state foo");
  equals(context.get('location'), 'foo/bar', "context.location should be 'foo/bar'");
  equals(context.get('params'), params, "context.params should be value passed to state.routeTriggered method");
  equals(context.get('handler'), foo.routeTriggered, "context.handler should be reference to state.routeTriggered");
...
test("invoke routeTriggered where delegate does not allow state to handle route", function() {
  ...
  ok(SC.kindOf(context, SC.AppSubstateRouteHandlerContext), "state.statechartStateCancelledHandlingTriggeredRoute should be provided a state route handler context object");
  equals(context.get('state'), foo, "context.state should be state foo");
  equals(context.get('location'), 'foo/bar', "context.location should be 'foo/bar'");
  equals(context.get('params'), params, "context.params should be value passed to state.routeTriggered method");
  equals(context.get('handler'), foo.routeTriggered, "context.handler should be reference to state.routeTriggered");
