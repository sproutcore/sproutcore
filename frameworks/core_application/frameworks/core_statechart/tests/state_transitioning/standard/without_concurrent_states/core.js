...
test("go to state b and then go to root state", function() {
  ...
  monitor.reset();
  statechart.gotoSubstate(statechart.get('rootSubstate'));

  var root = statechart.get('rootSubstate');
...
test("from state g, go to state m calling state g\'s gotoSubstate", function() {
  equals(stateG.get('isCurrentState'), true, 'state g should be current state');
  equals(stateM.get('isCurrentState'), false, 'state m should not be current state');

  monitor.reset();
  stateG.gotoSubstate('m');
