...
/**
  ...
  The following example shows how states are nested within a statechart:
      ...
      MyApp.Statechart = SC.Object.extend(SC.StatechartManager, {
        rootSubstate: SC.Substate.extend({
          ...
          stateA: SC.Substate.extend({
            // ... can continue to nest further states
          }),

          stateB: SC.Substate.extend({
            // ... can continue to nest further states
          })
        })
      });
  ...
  by the mixin and make the states on the object substates of the root state. As an example:
      ...
      MyApp.Statechart = SC.Object.extend(SC.StatechartManager, {
        ...
        stateA: SC.Substate.design({
          // ... can continue to nest further states
        }),

        stateB: SC.Substate.design({
          // ... can continue to nest further states
        })
      });

  If you liked to specify a class that should be used as the root state but using the above method to defined
  states, you can set the rootSubstateExample property with a class that extends from SC.Substate. If the
  rootSubstateExample property is not explicitly assigned the then default class used will be SC.Substate.
      ...
      MyApp.Statechart = SC.Object.extend(SC.StatechartManager, {
        rootSubstate: SC.Substate.design({
          ...
          stateA: SC.Substate.design({
            ...
            stateM: SC.Substate.design({ ... })
            stateN: SC.Substate.design({ ... })
            stateO: SC.Substate.design({ ... })
          }),

          stateB: SC.Substate.design({
            initialSubstate: 'stateX',

            stateX: SC.Substate.design({ ... })
            stateY: SC.Substate.design({ ... })
          })
        })
      });
  ...
  within one file. We could update the code and split the logic across two or more files like so:
      ...
      MyApp.StateA = SC.Substate.extend({
        substatesAreConcurrent: YES,
      ...
      MyApp.StateB = SC.Substate.extend({
        substatesAreConcurrent: YES,
      ...
      MyApp.Statechart = SC.Object.extend(SC.StatechartManager, {
        rootSubstate: SC.Substate.design({
          initialSubstate: 'stateA',
          stateA: SC.Substate.plugin('MyApp.StateA'),
          stateB: SC.Substate.plugin('MyApp.StateB')
        })
      });
...
SC.StatechartManager = /** @scope SC.StatechartManager.prototype */{
  ...
  */
  ...
  /**
    ...
    it will used the rootSubstateExample, initialSubstate, and statesAreConcurrent
    properties to construct a root state.

    @see #rootSubstateExample
    @see #initialSubstate
    @see #statesAreConcurrent

    @property {SC.Substate}
  */
  rootSubstate: null,

  /**
    Represents the class used to construct a class that will be the root state for
    this statechart. The class assigned must derive from SC.Substate.
    ...
    @property {SC.Substate}
  */
  rootSubstateExample: SC.Substate,
  ...
  */
  ...
  /**
    ...
    initialSubstate property must not be assigned.
  ...
  */
  ...
  destroyMixin: function () {
    var root = this.get('rootSubstate');
  ...
  */
  initStatechart: function () {
    ...
    var rootSubstate = this.get('rootSubstate'),
        msg;

    // If no root state was explicitly defined then try to construct
    // a root state class
    if (!rootSubstate) {
      rootSubstate = this._constructRootStateClass();
    }
    else if (SC.typeOf(rootSubstate) === SC.T_FUNCTION && rootSubstate.statePlugin) {
      rootSubstate = rootSubstate.apply(this);
    }

    if (!(SC.kindOf(rootSubstate, SC.Substate) && rootSubstate.isClass)) {
      msg = "Unable to initialize statechart. Root state must be a state class";
      this.statechartLogError(msg);
      throw msg;
    }

    rootSubstate = this.createRootState(rootSubstate, {
      statechart: this,
      name: SC.ROOT_SUBSTATE_NAME
    });

    this.set('rootSubstate', rootSubstate);
    rootSubstate.initState();

    if (SC.kindOf(rootSubstate.get('initialSubstate'), SC.EmptyState)) {
      msg = "Unable to initialize statechart. Root state must have an initial substate explicitly defined";
      this.statechartLogError(msg);
      throw msg;
    }

    if (!SC.empty(this.get('initialSubstate'))) {
      var key = 'initialSubstate';
      this.set(key, rootSubstate.get(this.get(key)));
    }

    this.set('statechartIsInitialized', YES);
    this.gotoState(rootSubstate);
  ...
  */
  currentStates: function () {
    return this.getPath('rootSubstate.currentSubstates');
  }.property().cacheable(),
  ...
  */
  ...
  /**
    ...
    where <state> is either a SC.Substate object or a string and <hash> is a regular JS hash object.

    @param state {SC.Substate|String} the state to go to (may not be the final state in the transition process)
    @param fromCurrentState {SC.Substate|String} Optional. The current state to start the transition process from.
  ...
  */
  ...
  /**
    ...
    @param state {SC.Substate|String} the state to go to and follow it's history state
    @param fromCurrentState {SC.Substate|String} Optional. the current state to start the state transition process from
  ...
  */
  ...
  /**
    ...
    @param {SC.Substate} state the state that will try to handle the event
  ...
  */
  ...
  /**
    ...
    @param {SC.Substate} state the state that did try to handle the event
  ...
  */
  _processGotoStateArgs: function (args) {
    ...
    if (len === 2) {
      ...
      case SC.T_HASH:
      case SC.T_OBJECT:
        if (!SC.kindOf(value, SC.Substate)) {
  ...
  /** @private
    ...
    it based on properties found on this state that derive from a SC.Substate class. For the
    root state to be successfully built, the following much be met:

     - The rootSubstateExample property must be defined with a class that derives from SC.Substate
     - Either the initialSubstate or statesAreConcurrent property must be set, but not both
     - There must be one or more states that can be added to the root state

  */
  _constructRootStateClass: function () {
    var rsExampleKey = 'rootSubstateExample',
    ...
    if (!(SC.kindOf(rsExample, SC.Substate) && rsExample.isClass)) {
      this._logStatechartCreationError("Invalid root state example");
      return null;
    }

    if (statesAreConcurrent && !SC.empty(initialSubstate)) {
    ...
    } else if (SC.typeOf(initialSubstate) === SC.T_STRING) {
      attrs.initialSubstate = initialSubstate;
    } else {
...
*/
SC.ROOT_SUBSTATE_NAME = "__ROOT_SUBSTATE__";
