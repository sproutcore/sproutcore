...
/** @scope SC.Scanner.prototype */
{
   ...
   * characters are processed. The default is 0: the beginning of the string.
   * 
   * @type Integer
   */
  scanLocation: 0,
   ...
   * 
   * @param {Integer}
   *          len The amount of characters to read
  ...
  scan: function(len) {
    if(this.scanLocation + len > this.length)
      throw SC.SCANNER_OUT_OF_BOUNDS_ERROR;
   ...
   * 
   * @param {Integer}
   *          min_len The minimum amount of characters to read
   * @param {Integer}
   *          [max_len] The maximum amount of characters to read (defaults to
   *          the minimum)
   ...
   * @returns {Boolean} YES if the given string was successfully scanned, NO
   *          otherwise
   */
   ...
   * 
   * @param {Array}
   *          ary the array of strings to scan
 ...
 * supports time zones other than UTC and that local to the machine on which it
 * is running. Any time zone can be specified when creating an `SC.DateTime`
 * object, e.g.
 ...
 * var e = SC.DateTime.create({ hour: 10, timezone: 0 }); // same time,
 * specified in UTC
 * 
 * and it is true that `d.isEqual(e)`.
 * 
 * The time zone specified upon creation is permanent, and any calls to `get()`
 * on that instance will return values expressed in that time zone. So,
 * 
 * d.get('hour') returns 5. e.get('hour') returns 10.
 * 
 * but
 * 
 ...
 * 
 * SC.Record.attr(SC.DateTime); // Default format is ISO8601. See
 * `SC.DateTime.recordFormat` SC.Record.attr(SC.DateTime, { format: '%d/%m/%Y'
 * }); // Attribute stored as a string in '%d/%m/%Y' format
 * SC.Record.attr(SC.DateTime, { useUnixTime: YES }); // Attribute stored as a
 * number in Unix time
 * 
 * @extends SC.Object
...
/** @scope SC.DateTime.prototype */
{
   ...
   * @private
   * 
   * Internal representation of a date: the number of milliseconds since
  ...
  /**
   * @read-only The offset, in minutes, between UTC and the object's timezone.
   *            All calls to `get()` will use this time zone to translate
   *            date/time values into the zone specified here.
   * 
   * @type Integer
   */
  timezone: 0,
   ...
   * assumed to be local to it, and the returned `SC.DateTime` instance has that
   * time zone. If none is passed, it defaults to `SC.DateTime.timezone`.
   * 
   ...
   * parameters. Don't use floating point values, it might give unpredictable
   * results.
   * 
   * @see SC.DateTime#create for the list of options you can pass
   * @param {Hash}
   *          options the amount of date/time to advance the receiver
   ...
   * Generic getter.
   * 
   * The properties you can get are: - `year` - `month` (January is 1, contrary
   * to JavaScript Dates for which January is 0) - `day` - `dayOfWeek` (Sunday
   * is 0) - `hour` - `minute` - `second` - `millisecond` - `milliseconds`, the
   * number of milliseconds since January, 1st 1970 00:00:00.0 UTC - `elapsed`,
   * the number of milliseconds until (-), or since (+), the date. -
   * `isLeapYear`, a boolean value indicating whether the receiver's year is a
   * leap year - `daysInMonth`, the number of days of the receiver's current
   * month - `dayOfYear`, January 1st is 1, December 31th is 365 for a common
   * year - `week` or `week1`, the week number of the current year, starting
   * with the first Sunday as the first day of the first week (00..53) -
   * `week0`, the week number of the current year, starting with the first
   * Monday as the first day of the first week (00..53) - `lastMonday`,
   * `lastTuesday`, etc., `nextMonday`, `nextTuesday`, etc., the date of the
   * last or next weekday in comparison to the receiver.
   * 
   * @param {String}
   ...
   * 
   * The format parameter can contain the following characters: - %a -- The
   * abbreviated weekday name (``Sun'') - %A -- The full weekday name
   * (``Sunday'') - %b -- The abbreviated month name (``Jan'') - %B -- The full
   * month name (``January'') - %c -- The preferred local date and time
   * representation - %d -- Day of the month (01..31) - %D -- Day of the month
   * (0..31) - %e -- The calculated elapsed time integer for the smallest
   * relevant time interval. Note that this is a special formatting character,
   * and will only be recognized in combination with the %E formatter. - %E --
   * Elapsed time, according to localized text formatting strings. See below. -
   * %h -- Hour of the day, 24-hour clock (0..23) - %H -- Hour of the day,
   * 24-hour clock (00..23) - %i -- Hour of the day, 12-hour clock (1..12) - %I --
   * Hour of the day, 12-hour clock (01..12) - %j -- Day of the year (001..366) -
   * %m -- Month of the year (01..12) - %M -- Minute of the hour (00..59) - %p --
   * Meridian indicator (``AM'' or ``PM'') - %S -- Second of the minute (00..60) -
   * %s -- Milliseconds of the second (000..999) - %U -- Week number of the
   * current year, starting with the first Sunday as the first day of the first
   * week (00..53) - %W -- Week number of the current year, starting with the
   * first Monday as the first day of the first week (00..53) - %w -- Day of the
   * week (Sunday is 0, 0..6) - %x -- Preferred representation for the date
   * alone, no time - %X -- Preferred representation for the time alone, no date -
   * %y -- Year without a century (00..99) - %Y -- Year with century - %Z --
   * Time zone (ISO 8601 formatted) - %% -- Literal ``%'' character
   * 
   * The Elapsed date format is a special, SproutCore specific formatting
   * feature which will return an accurate-ish, human readable indication of
   * elapsed time. For example, it might return "In a minute", or "A year ago".
   * To customize the output for this formatter, override the date localization
   * strings in your app. It is only in this special case that the "%e"
   * parameter will be recognized; it will be replaced with the number of
   * intervals (seconds, minutes, weeks, etc) that are appropriate.
   * 
   * Examples when using %E: - "_SC.DateTime.secondsIn" -> "In %e seconds" ->
   * "In 20 seconds" - "_SC.DateTime.dayIn" -> "Tomorrow at %i:%M %p" ->
   ...
   */
  toString: function() {
    return "UTC: " + new Date(this._ms).toUTCString() + ", timezone: " + this.timezone;
  },
  
  /**
   * Returns `YES` if the passed `SC.DateTime` is equal to the receiver, ie: if
   * their number of milliseconds since January, 1st 1970 00:00:00.0 UTC are
   * equal. This is the preferred method for testing equality.
   * 
   * @see SC.DateTime#compare
   * @param {SC.DateTime}
   *          aDateTime the DateTime to compare to
   * @returns {Boolean}
   */
  ...
  /**
   * Returns a copy of the receiver. Because of the way `SC.DateTime` is
   * designed, it just returns the receiver.
   * 
   * @returns {SC.DateTime}
   */
  copy: function() {
   ...
   * 
   * If you don't pass any argument, the target timezone is assumed to be 0, ie
   * UTC.
   * 
   * Note that this method does not change the underlying position in time, but
...
/** @scope SC.DateTime */
{
   ...
   * @private
   * 
   * The English day names used for the 'lastMonday', 'nextTuesday', etc.,
   * getters.
   * 
   * @type Array
   */
   ...
   * @private
   * 
   * A cache of `SC.DateTime` instances. If you attempt to create a
   * `SC.DateTime` instance that has already been created, then it will return
   * the cached value.
   * 
   * @type Array
   */
   ...
   * 
   * 'milliseconds' is time since Jan 1, 1970. 'timezone' is the current time
   * zone we want to be working in internally.
   * 
   * Returns a hash of the previous milliseconds and time zone in case they are
   * wanted for later restoration.
   */
   ...
   * @private
   * 
   * By this time, any time zone setting on 'hash' will be ignored. 'timezone'
   * will be used, or the last this._tz.
   */
  _setCalcStateFromHash: function(hash, timezone) {
    var tz = (timezone !== undefined) ? timezone : this._tz; // use the
                                                              // last-known time
                                                              // zone if
                                                              // necessary
    var ms = this._toMilliseconds(hash, this._ms, tz); // convert the hash
                                                        // (local to specified
                                                        // time zone) to
                                                        // milliseconds (in UTC)
  ...
  _get: function(key, start, timezone) {
    ...
    // Set up an absolute date/time using the given milliseconds since Jan 1,
    // 1970.
    // Only do it if we're given a time value, though, otherwise we want to use
    // the
    // last one we had because this `_get()` method is recursive.
    //
    // Note that because these private time calc methods are recursive, and
    // because all DateTime instances
    // share an internal this._date and `this._tz` state for doing calculations,
    // methods
    // that modify `this._date` or `this._tz` should restore the last state
    // before exiting
    // to avoid obscure calculation bugs. So we save the original state here,
    // and restore
    // it before returning at the end.
    originalTime = this._setCalcState(start, timezone); // save so we can
                                                        // restore it to how it
                                                        // was before we got
                                                        // here
    
    // Check this first because it is an absolute value -- no tweaks necessary
    // when calling for milliseconds
    if(key === 'milliseconds') {
      v = d.getTime();
    ...
    if(v === null) {
      ...
      if(prefix === 'last' || prefix === 'next') {
        ...
        if(targetWeekday >= 0) {
          ...
          if(prefix === 'last' && delta >= 0)
            delta -= 7;
          if(prefix === 'next' && delta < 0)
            delta += 7;
          this._advance({
            day: delta
          }, start, timezone);
    ...
    if(v === null) {
      ...
      // Before calculating, we need to get everything into a common time zone
      // to
      // negate the effects of local machine time (so we can use all the
      // 'getUTC...() methods on Date).
      if(timezone !== undefined) {
        this._setCalcState(d.getTime() - (timezone * 60000), 0); // make this
                                                                  // instance's
                                                                  // time zone
                                                                  // the new UTC
                                                                  // temporarily
      ...
      switch(key) {
      case 'year':
        v = d.getUTCFullYear(); // TODO: investigate why some libraries do
                                // getFullYear().toString() or getFullYear()+""
        break;
      case 'month':
  ...
  _advance: function(options, start, timezone) {
    ...
    tz = (opts.timezone !== undefined) ? opts.timezone : timezone; // watch out
                                                                    // for zero,
                                                                    // which is
                                                                    // acceptable
                                                                    // as a time
   ...
   * @private
   * 
   * Converts a standard date/time options hash to an integer representing that
   * position in time relative to Jan 1, 1970
   */
  _toMilliseconds: function(options, start, timezone, resetCascadingly) {
    var opts = options ? SC.clone(options) : {};
    var d = this._date;
    var previousMilliseconds = d.getTime(); // rather than create a new Date
                                            // object, we'll reuse the instance
                                            // we have for calculations, then
                                            // restore it
    var ms, tz;
    
    // Initialize our internal for-calculations Date object to our current
    // date/time.
    // Note that this object was created in the local machine time zone, so when
    // we set
    // its params later, it will be assuming these values to be in the same time
    // zone as it is.
    // It's ok for start to be null, in which case we'll just keep whatever we
    // had in 'd' before.
    if(!SC.none(start)) {
      d.setTime(start); // using milliseconds here specifies an absolute
                        // location in time, regardless of time zone, so that's
                        // nice
    }
    
    // We have to get all time expressions, both in 'options' (assume to be in
    // time zone 'timezone')
    // and in 'd', to the same time zone before we can any calculations
    // correctly. So because the Date object provides
    // a suite of UTC getters and setters, we'll temporarily redefine 'timezone'
    // as our new
    // 'UTC', so we don't have to worry about local machine time. We do this by
    // subtracting
    // milliseconds for the time zone offset. Then we'll do all our
    // calculations, then convert
    // it back to real UTC.
    
    // (Zero time zone is considered a valid value.)
    tz = (timezone !== undefined) ? timezone : (this.timezone !== undefined) ? this.timezone : 0;
    d.setTime(d.getTime() - (tz * 60000)); // redefine 'UTC' to establish a new
                                            // local absolute so we can use all
                                            // the 'getUTC...()' Date methods
    ...
    // Since everything is in 'UTC' now, use the UTC accessors. We do this
    // because,
    // according to javascript Date spec, you have to set year, month, and day
    // together
    // if you're setting any one of them. So we'll use the provided Date.UTC()
    // method
    // to get milliseconds, and we need to get any missing values first...
    if(SC.none(opts.year))
      opts.year = d.getUTCFullYear();
    if(SC.none(opts.month))
      opts.month = d.getUTCMonth() + 1; // January is 0 in JavaScript
    if(SC.none(opts.day))
      opts.day = d.getUTCDate();
    if(SC.none(opts.hour))
      opts.hour = d.getUTCHours();
    if(SC.none(opts.minute))
      opts.minute = d.getUTCMinutes();
    if(SC.none(opts.second))
      opts.second = d.getUTCSeconds();
    if(SC.none(opts.millisecond))
      opts.millisecond = d.getUTCMilliseconds();
    
    // Ask the JS Date to calculate milliseconds for us (still in redefined
    // UTC). It
    // is best to set them all together because, for example, a day value means
    // different things
    // to the JS Date object depending on which month or year it is. It can now
    // handle that stuff
    ...
    // Now that we've done all our calculations in a common time zone, add back
    // the offset
    // to move back to real UTC.
    d.setTime(ms + (tz * 60000));
    ms = d.getTime(); // now get the corrected milliseconds value
    
    // Restore what was there previously before leaving in case someone called
    // this method
   ...
   *  - none, to create a `SC.DateTime` instance initialized to the current date
   * and time in the local timezone, - a integer, the number of milliseconds
   * since January, 1st 1970 00:00:00.0 UTC - a options hash that can contain
   * any of the following properties: year, month, day, hour, minute, second,
   * millisecond, timezone
   * 
   * Note that if you attempt to create a `SC.DateTime` instance that has
   * already been created, then, for performance reasons, a cached value may be
   * returned.
   * 
  ...
  /**
   * Returns a `SC.DateTime` object created from a given string parsed with a
   * given format. Returns `null` if the parsing fails.
   * 
   * @see SC.DateTime#toFormattedString for a description of the format
   *      parameter
   * @param {String}
   *          str the string to parse
   * @param {String}
   *          fmt the format to parse the string with
  ...
  parse: function(str, fmt) {
    ...
    var d, parts, opts = {}, check = {}, scanner = SC.Scanner.create({
      string: str
    });
    
    if(SC.none(fmt))
      fmt = SC.DATETIME_ISO8601;
    
    try {
      while((parts = re.exec(fmt)) !== null) {
        switch(parts[1]) {
        case 'a':
          check.dayOfWeek = scanner.scanArray(this.abbreviatedDayNames);
          break;
        case 'A':
          check.dayOfWeek = scanner.scanArray(this.dayNames);
          break;
        case 'b':
          opts.month = scanner.scanArray(this.abbreviatedMonthNames) + 1;
          break;
        case 'B':
          opts.month = scanner.scanArray(this.monthNames) + 1;
          break;
        case 'c':
          throw "%c is not implemented";
        case 'd':
        case 'D':
          opts.day = scanner.scanInt(1, 2);
          break;
        case 'e':
          throw "%e is not implemented";
        case 'E':
          throw "%E is not implemented";
        case 'h':
        case 'H':
          opts.hour = scanner.scanInt(1, 2);
          break;
        case 'i':
        case 'I':
          opts.hour = scanner.scanInt(1, 2);
          break;
        case 'j':
          throw "%j is not implemented";
        case 'm':
          opts.month = scanner.scanInt(1, 2);
          break;
        case 'M':
          opts.minute = scanner.scanInt(1, 2);
          break;
        case 'p':
          opts.meridian = scanner.scanArray(this.AMPMNames);
          break;
        case 'S':
          opts.second = scanner.scanInt(1, 2);
          break;
        case 's':
          opts.millisecond = scanner.scanInt(1, 3);
          break;
        case 'U':
          throw "%U is not implemented";
        case 'W':
          throw "%W is not implemented";
        case 'w':
          throw "%w is not implemented";
        case 'x':
          throw "%x is not implemented";
        case 'X':
          throw "%X is not implemented";
        case 'y':
          opts.year = scanner.scanInt(2);
          opts.year += (opts.year > 70 ? 1900 : 2000);
          break;
        case 'Y':
          opts.year = scanner.scanInt(4);
          break;
        case 'Z':
    ...
    if(!SC.none(opts.meridian) && !SC.none(opts.hour)) {
      if((opts.meridian === 1 && opts.hour !== 12) || (opts.meridian === 0 && opts.hour === 12)) {
   ...
   * @private
   * 
   * Converts the x parameter into a string padded with 0s so that the
   * string’s length is at least equal to the len parameter.
   * 
   * @param {Object}
  ...
  __toFormattedString: function(part, start, timezone) {
    ...
    //
    // This method is simply a helper for this._toFormattedString() (one
    // underscore);
    ...
    switch(part[1]) {
    case 'a':
      return this.abbreviatedDayNames[this._get('dayOfWeek')];
    case 'A':
      return this.dayNames[this._get('dayOfWeek')];
    case 'b':
      return this.abbreviatedMonthNames[this._get('month') - 1];
    case 'B':
      return this.monthNames[this._get('month') - 1];
    case 'c':
      return this._date.toString();
    case 'd':
      return this._pad(this._get('day'));
    case 'D':
      return this._get('day');
    case 'E':
      return this._toFormattedString(this.__getElapsedStringFormat(start, timezone), start, timezone);
    case 'h':
      return this._get('hour');
    case 'H':
      return this._pad(this._get('hour'));
    case 'i':
      hour = this._get('hour');
    ...
    case 'j':
      return this._pad(this._get('dayOfYear'), 3);
    case 'm':
      return this._pad(this._get('month'));
    case 'M':
      return this._pad(this._get('minute'));
    case 'p':
      return this._get('hour') > 11 ? this.AMPMNames[1] : this.AMPMNames[0];
    case 'S':
      return this._pad(this._get('second'));
    case 's':
      return this._pad(this._get('millisecond'), 3);
    case 'u':
      return this._pad(this._get('utc')); // utc
    case 'U':
      return this._pad(this._get('week0'));
    case 'W':
      return this._pad(this._get('week1'));
    case 'w':
      return this._get('dayOfWeek');
    case 'x':
      return this._date.toDateString();
    case 'X':
      return this._date.toTimeString();
    case 'y':
      return this._pad(this._get('year') % 100);
    case 'Y':
      return this._get('year');
    case 'Z':
      offset = -1 * timezone;
      return (offset >= 0 ? '+' : '-') + this._pad(parseInt(Math.abs(offset) / 60, 10)) + ':' + this._pad(Math.abs(offset) % 60);
    case '%':
      return '%';
    }
  },
  ...
  _toFormattedString: function(format, start, timezone) {
    ...
    this._setCalcState(start - (timezone * 60000), 0); // so simulate a shifted
                                                        // 'UTC' time
    
    return format.replace(/\%([aAbBcdEDhHiIjmMpsSUWwxXyYZ\%])/g, function() {
   ...
   * comparing their number of milliseconds since January, 1st 1970 00:00:00.0
   * UTC.
   * 
   * @param {SC.DateTime}
   *          a the first DateTime instance
   * @param {SC.DateTime}
   *          b the second DateTime instance
  ...
  /**
   * This will tell you which of the two passed DateTime is greater by only
   * comparing the date parts of the passed objects. Only dates with the same
   * timezone can be compared.
   * 
   * @param {SC.DateTime}
   *          a the first DateTime instance
   * @param {SC.DateTime}
   *          b the second DateTime instance
   * @returns {Integer} -1 if a < b, +1 if a > b, 0 if a == b
   * @throws {SC.DATETIME_COMPAREDATE_TIMEZONE_ERROR}
   *           if the passed arguments don't have the same timezone
   */
  compareDate: function(a, b) {
    if(a.get('timezone') !== b.get('timezone'))
      throw SC.DATETIME_COMPAREDATE_TIMEZONE_ERROR;
    var ma = a.adjust({
      hour: 0
    }).get('milliseconds');
    var mb = b.adjust({
...
/**
 * Adds a transform to format the DateTime value to a String value according to
