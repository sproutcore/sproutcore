...
var runInContext = function (code, filename) {
  ...
  if (filename !== undefined) {
    ...
    // util.log('runInContext: original dirname: ' + oldDir);
    // util.log('runInContext: original filename: ' + oldFile);
    // util.log('runInContext: file to run: ' + context.__filename);
    // util.log('runInContext: dirname: ' + context.__dirname);
  }
  try {
  ...
  if (oldDir || oldFile) {
    //util.log("setting back original dirname and filename");
    context.__dirname = oldDir;
    context.__filename = oldFile;
    // util.log('runInContext: file turning back to: ' + context.__filename);
    // util.log('runInContext: dirname: ' + context.__dirname);
  }
};
