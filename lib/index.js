...
var scanDir = function (dir, baseDir) {
  var ret = [];
  ...
  fileList.forEach(function (fn) {
    ...
    if (stat.isFile() && isJS) {
      ...
      ret = ret.concat(scanDir(p, baseDir));
    }
  });
  return ret;
};
...
fws.forEach(function (fw) {
  files[fw] = scanDir(fw, fw);
  files[fw] = sortFilesByRequirements(files[fw], [pathlib.join(fw, "core.js")], [], sortOrder);
  files[fw].forEach(function (f) {
    var c = "var window = this; \n " + files[f];
