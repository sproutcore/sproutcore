...
var sortFilesByRequirements = function (files, beginWithFiles, endWithFiles, dependencies) {
  ...
  */
  var recurser = function (fileOrFolder) {
    var i, len, folderLen;
    if (fileOrFolder.slice(-1) === pathlib.sep) { // Handle folders.
    ...
    // For files, check if it's been handled; if not, recurse its dependencies.
      ...
      if (ret.indexOf(fileOrFolder) === -1) {  // If the file hasn't been processed yet...
        if (currentlyProcessingFiles.indexOf(fileOrFolder) > -1) { // Check for circularity and error out if found.
          console.log("BuildTools encountered a circular dependency.");
          console.log('The file ' + fileOrFolder + ' was required via sc_require(), while already being processed:');
          currentlyProcessingFiles.forEach(function (file) { console.log('  ' + file + ' =>'); });
...
var runInContext = function (code, filename) {
  ...
  if (filename !== undefined) {
    ...
    // console.log('runInContext: original dirname: ' + oldDir);
    // console.log('runInContext: original filename: ' + oldFile);
    // console.log('runInContext: file to run: ' + context.__filename);
    // console.log('runInContext: dirname: ' + context.__dirname);
  }
  try {
    //code = "__dirname = '" + pathlib.dirname(filename) + "';\n" + code;
    vm.runInContext(code, context, filename);
  }
  catch (e) {
    console.log('Evaluation error in ' + filename + ':' + util.inspect(e));
    console.log('error.errorcode: ' + util.inspect(e.message));
    // throw it again to give a proper stack trace, otherwise the original source file
    // cannot be determined
    throw e;
  }
  if (oldDir || oldFile) {
    //console.log("setting back original dirname and filename");
    context.__dirname = oldDir;
    context.__filename = oldFile;
    // console.log('runInContext: file turning back to: ' + context.__filename);
    // console.log('runInContext: dirname: ' + context.__dirname);
  }
};
...
var ctx = {
  ...
  document: {
    ...
    createElement: function () {
      return { style: { animation: 0, transition: 0, transform: 0, perspective: 0 } };
    },
      ...
      if (ret.isError) {
        if (ret.code === "ENOENT") { // file doesn't exist
          if (abspath.lastIndexOf('.js') === -1) {
            abspath = abspath + ".js";
            //console.log("trying with .js attached, because the ref doesn't have it: " + abspath);
            if (filesLoadedInContext.indexOf(abspath) === -1) {
              //console.log('file ' + abspath + " has not been loaded before, loading...");
              // try to load in place
              ret = loadFile(abspath);
              if (ret.isError) {
                //console.log('inside catch block of sc_require: ' + util.inspect(ret));
                //console.log('sc_require is called from ' + insideFile);
                if (ret.code === "ENOENT") {
                  //console.log(" file " + abspath + " doesn't seem to exist");
                  return ret;
                }
                else throw ret; // throw if it is something else from ENOENT
              }
              //console.log('file ' + abspath + " successfully loaded");
            }
            else {
              //console.log("didn't try to load " + abspath + " because it is already loaded");
            }
          }
        }
      }
...
var startUp = function () {
  ...
  //console.log("filesLoaded: " + util.inspect(filesLoadedInContext));
  runInContext("SC.isReady = true;"); // make sure that things start working
};
