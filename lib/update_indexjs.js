...
function findAndSortFiles (directory, options) {
  ...
  function scanDir (dir) {
    ...
    fileList.forEach(function (fn) {
      ...
      if (stat.isFile()) {
        if (p.includes("webpack")) return;
        if (p.endsWith("index.js")) return; // do not include an existing index
        if (p.endsWith("webpack.config.js")) return; // skip file
        if (p.endsWith(".css") || p.endsWith(".scss")) {
          if (skipDirs.some(sd => p.includes(sd))) return;
...
if (instanceof commander.version !== "function") {
  // for more recent versions of commander / ES6
  commander = commander.program;
}
